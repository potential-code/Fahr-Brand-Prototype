...
const ASSURANCES = [
  { icon: ShieldCheck, key: "landing.cta.assurances.governance" },
  { icon: Lock, key: "landing.cta.assurances.residency" },
  { icon: FileCheck2, key: "landing.cta.assurances.pdpl" },
  { icon: Sparkles, key: "landing.cta.assurances.bilingual" },
];
    ...
    <section className={cn("relative overflow-hidden border-b border-border", TYPE.section)}>
      ...
      <div className={cn("relative z-10 text-center", TYPE.gutter)}>
        <RevealHeading
          as="h2"
          segments={[
            t("landing.cta.headline.line1"),
            { t: t("landing.cta.headline.accent"), accent: true },
          ]}
        ...
        <Reveal variant="up" delay={0.18} className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row">
          ...
          >
            <Link href="/login">{t("landing.cta.ctaSecondary")}</Link>
          </Button>
        </Reveal>
        ...
        <Reveal variant="fade" delay={0.3}>
          <ul className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2.5">
            {ASSURANCES.map(({ icon: Icon, key }) => (
              <li key={key} className="inline-flex items-center gap-2 text-[11px] font-medium text-white/70 md:text-xs">
