    ...
    <ol className="relative space-y-3">
      {entries.map(({ item, status }, i) => {
        ...
        return (
          ...
          >
            {/* Connector + node */}
            {!isLast && <span className="absolute start-[19px] top-11 bottom-[-0.75rem] w-px bg-border" />}
            <span
              className={`absolute start-0 top-4 flex h-10 w-10 items-center justify-center rounded-full border ${
                status === "completed"
