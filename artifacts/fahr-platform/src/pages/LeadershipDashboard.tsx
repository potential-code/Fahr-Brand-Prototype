      ...
      <PageEnter className="space-y-8 pb-12">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Top workforce readiness challenges requiring federal coordination. Click any row to jump to the most affected entity.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-end">Affected Ministries</TableHead>
                    <TableHead className="text-end">Trend</TableHead>
                  </TableRow>
                </TableHeader>
                <Stagger as="tbody" gap={0.1}>
                  {gaps.map((risk) => {
                    // Find first ministry with this gap for the click target
                    const firstMinistryWithGap = ministries.find(m => m.topGapCompetencyId === risk.competency.id);
                    
                    return (
                      <StaggerItem 
                        as="tr" 
                        key={risk.competency.id} 
                        className={firstMinistryWithGap ? "cursor-pointer hover:bg-muted/50 transition-colors focus-visible:bg-muted/60 focus-visible:outline-none" : ""}
                      >
                        <TableCell 
                          className="font-medium"
                          onClick={() => {
                            if (firstMinistryWithGap) setLocation(`/fahr?ministry=${firstMinistryWithGap.id}`);
                          }}
                        >
                          <button 
                            className="flex items-center gap-2 text-start focus-visible:outline-none"
                            tabIndex={firstMinistryWithGap ? 0 : -1}
                          >
